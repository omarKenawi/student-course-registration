...
export default function CreateInstructorPage() {
  ...
  const [form, setForm] = useState({ fullName: '', email: '', password: '', employeeNumber: '' })
  const [err, setErr] = useState('')

  const mut = useMutation({
    mutationFn: () => createInstructor(form),
    ...
    <div className="max-w-lg mx-auto space-y-4">
      ...
      <form onSubmit={(e) => { e.preventDefault(); mut.mutate() }} className="bg-white border rounded-lg p-6 space-y-4">
        ...
        <div>
          ...
          <input value={form.employeeNumber} onChange={set('employeeNumber')} required className="w-full border rounded-md px-3 py-2 text-sm" />
        </div>
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => navigate('/instructors')} className="px-4 py-2 border rounded-md text-sm hover:bg-gray-50">
            Cancel
          </button>
        </div>
      </form>
