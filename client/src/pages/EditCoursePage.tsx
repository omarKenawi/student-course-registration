...
export default function EditCoursePage() {
  ...
  const mut = useMutation({
    mutationFn: () => updateCourse(courseId, {
      ...
      term: form.term ? (form.term as Term) : undefined,
      academicYear: form.academicYear ? Number(form.academicYear) : undefined,
      academicLevel: form.academicLevel ? (form.academicLevel as AcademicLevel) : undefined,
      ...
      <form onSubmit={(e) => { e.preventDefault(); mut.mutate() }} className="bg-white border rounded-lg p-6 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input value={form.code} onChange={set('code')} className="w-full border rounded-md px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Credit Hours</label>
            <input type="number" min={1} value={form.creditHours} onChange={set('creditHours')} className="w-full border rounded-md px-3 py-2 text-sm" />
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700">Title</label>
          <input value={form.title} onChange={set('title')} className="w-full border rounded-md px-3 py-2 text-sm" />
        </div>
        <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" min={1} value={form.capacity} onChange={set('capacity')} className="w-full border rounded-md px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Academic Year</label>
            <input type="number" min={2024} value={form.academicYear} onChange={set('academicYear')} className="w-full border rounded-md px-3 py-2 text-sm" />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Term</label>
            <select value={form.term} onChange={set('term')} className="w-full border rounded-md px-3 py-2 text-sm">
          ...
          <div>
            ...
            <select value={form.academicLevel} onChange={set('academicLevel')} className="w-full border rounded-md px-3 py-2 text-sm">
              <option value="">-- Select --</option>
        ...
        <div>
          ...
          <select value={form.instructorId} onChange={set('instructorId')} className="w-full border rounded-md px-3 py-2 text-sm">
            <option value="">-- Select --</option>
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => navigate(`/courses/${courseId}`)} className="px-4 py-2 border rounded-md text-sm hover:bg-gray-50">Cancel</button>
        </div>
      </form>
    </div>
  )
